...
def train_model():
	sweep_config = dict(lr=1e-3, batch_size=64, regularization=5E-4)
	wandb.init(config=sweep_config)
    	config = wandb.config
	batch_size = config.batch_size
	ds_train, ds_val = load_data(batch_size, spark)   # at this point, we modified load_data in order to output only data with 200 classes
	...
	with strategy.scope():
		...
		model_MNV2 = add_regularization(model_MNV2, penalty=config.regularization)
		model_MNV2.compile(optimizer, loss='categorical_crossentropy', metrics=['accuracy', m])
	history = model_MNV2.fit(ds_train, validation_data=ds_val, epochs=epochs, verbose = 1)
	return history
